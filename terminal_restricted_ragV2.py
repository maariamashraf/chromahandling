...
class TerminalRestrictedRAG:
    def __init__(self, config_file: str = None):
        ...
        self.config = {
            'database': {
                'chroma_db_path': 'C:/Users/maria/Downloads/VectorSearch-main/VectorSearch-main/storage',
    ...
    def load_config(self, config_file: str):
        ...
        try:
            ...
            self.print_colored(f"✅ Configuration loaded from {config_file}", 'green')
        except Exception as e:
            self.print_colored(f"⚠️  Error loading config: {e}", 'yellow')
    ...
    def save_config(self, config_file: str = None):
        ...
        try:
            ...
            self.print_colored(f"💾 Configuration saved to {config_file}", 'green')
        except Exception as e:
            self.print_colored(f"❌ Error saving config: {e}", 'red')
    ...
    def initialize_models(self):
        ...
        try:
            self.print_colored("🔄 Initializing models...", 'blue')
            ...
            self.print_colored("✅ All models initialized successfully!", 'green')
            return True
            
        except Exception as e:
            self.print_colored(f"❌ Error initializing models: {e}", 'red')
            return False

    def terminal_setup_wizard(self):
        """Interactive setup wizard for first-time configuration"""
        self.print_colored("🎯 RAG System Setup Wizard", 'cyan', 'bold')
        self.print_colored("=" * 50, 'cyan')
        
        # Model configuration
        self.print_colored("\n📁 Model Configuration:", 'yellow', 'bold')
        ...
        self.print_colored("\n🗄️  Database Configuration:", 'yellow', 'bold')
        ...
        self.print_colored("\n🔒 Restriction Configuration:", 'yellow', 'bold')
        ...
        self.print_colored("\n🖥️  Terminal Preferences:", 'yellow', 'bold')
    ...
    def _add_user_role_restriction(self):
        ...
        if blocked_patterns:
            ...
            self.print_colored(f"✅ Added restrictions for role: {role}", 'green')
    ...
    def _add_single_file(self):
        ...
        try:
            ...
            self.print_colored(f"✅ Added {len(chunks)} chunks from {file_path}", 'green')
    ...
    def _add_from_directory(self):
        ...
        if not os.path.isdir(dir_path):
            self.print_colored("❌ Directory not found", 'red')
            return
            ...
            for file in files:
                if any(file.lower().endswith(ext) for ext in extensions):
                    ...
                    try:
                        ...
                        self.print_colored(f"⚠️  Error processing {file_path}: {e}", 'yellow')
        
        self.print_colored(f"✅ Processed {files_processed} files from {dir_path}", 'green')
    ...
    def _add_direct_text(self):
        ...
        if not content.strip():
            self.print_colored("❌ No content provided", 'red')
            return
        ...
        self._add_documents_to_db(chunks, metadatas, ids)
        self.print_colored(f"✅ Added {len(chunks)} chunks from direct input", 'green')
    ...
    def _is_document_allowed(self, document: str, metadata: Dict) -> bool:
        ...
        for keyword in restrictions['blocked_keywords']:
            if keyword.lower() in doc_lower:
                self.print_colored(f"⚠️  Document blocked due to keyword: {keyword}", 'yellow')
                return False
        ...
        if restrictions['allowed_domains']:
            ...
            if domain not in restrictions['allowed_domains']:
                self.print_colored(f"⚠️  Document blocked due to domain: {domain}", 'yellow')
                return False
    ...
    def query_interactive(self):
        ...
        if not self.model:
            self.print_colored("❌ Models not initialized. Run setup first.", 'red')
            return
        
        self.print_colored("\n💬 Interactive Query Mode", 'cyan', 'bold')
            ...
            try:
                ...
                if user_input.lower() == 'quit':
                    ...
                    if new_role:
                        ...
                        self.print_colored(f"✅ Role changed to: {new_role}", 'green')
                    continue
    ...
    def _process_query(self, query: str):
        ...
        self.print_colored("🤔 Processing...", 'blue')
        
        try:
            result = self.generate_response(query, self.current_user_role)
            
            if result['restricted']:
                self.print_colored("🚫 RESTRICTED RESPONSE", 'red', 'bold')
            ...
            self.print_colored(f"❌ Error processing query: {e}", 'red')
    ...
    def _retrieve_context(self, query: str) -> List[Dict]:
        ...
        try:
            ...
            self.print_colored(f"⚠️  Error retrieving context: {e}", 'yellow')
            return []

    def main_menu(self):
        """Main terminal interface"""
        self.print_colored("🚀 Terminal Restricted RAG System", 'bright_blue', 'bold')
        ...
        while True:
            ...
            if choice == '1':
                if not self.initialize_models():
                    ...
                    if not self.initialize_models():
                        self.print_colored("❌ Failed to initialize system", 'red')
            elif choice == '2':
                if not self.collection:
                    self.print_colored("❌ Initialize system first", 'red')
                else:
                ...
                self.print_colored("\n👋 Thank you for using Terminal RAG System!", 'cyan')
                if self.config['terminal']['auto_save_config'] and self.config_file:
                    self.save_config()
                    self.print_colored("💾 Configuration auto-saved", 'green')
                break
            else:
    ...
    def _show_configuration(self):
        ...
        # Database settings
        self.print_colored("\n📁 Database Settings:", 'yellow', 'bold')
        db_config = self.config['database']
        for key, value in db_config.items():
            self.print_colored(f"  {key}: {value}", 'white')
        
        # Model settings
        self.print_colored("\n🤖 Model Settings:", 'yellow', 'bold')
        model_config = self.config['model']
        for key, value in model_config.items():
            self.print_colored(f"  {key}: {value}", 'white')
        
        # Restrictions
        self.print_colored("\n🔒 Active Restrictions:", 'yellow', 'bold')
        restrictions = self.config['restrictions']
        ...
        # Terminal settings
        self.print_colored("\n🖥️  Terminal Settings:", 'yellow', 'bold')
    ...
    try:
        ...
        if args.setup:
            ...
            if not rag_system.initialize_models():
                rag_system.print_colored("❌ Failed to initialize system", 'red')
                return 1
            ...
            try:
                ...
                rag_system.print_colored(f"❌ Batch file not found: {args.batch_file}", 'red')
                return 1
        ...
        rag_system.print_colored("\n\n👋 Interrupted by user. Goodbye!", 'cyan')
        return 0
    except Exception as e:
        rag_system.print_colored(f"❌ Unexpected error: {e}", 'red')
        return 1
